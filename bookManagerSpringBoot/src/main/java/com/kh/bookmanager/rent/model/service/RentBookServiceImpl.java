...
public class RentBookServiceImpl implements RentBookService{
	...
	public Map<String,Object> selectRentinfo(String userId, int currentPage, int cntPerPage){
		...
		System.out.println(">>>>>>>" + rdao.selectRentinfo(commandMap));
		commandMap.put("data",rdao.selectRentinfo(commandMap));
		return commandMap;
	}
	
	public List<RentBook> selectRentinfo(Map<String,Object> commandMap) {
		List<RentBook> rentInfo = rdao.selectRentbookinfo(commandMap);
		for(RentBook b : rentInfo) {
	...
	public void insertRentBookInfo(Rent rent) {
		...
		//도서재고가 없어서 대출 불가능
		if(bookDao.selectBookAmt(rent.getBook().getbIdx()) < 1 ){
	...
	public void updateExtendRentBook(Rent rent, String memberGrade)  {
		...
		//연장횟수 확인
		List<RentBook> curRentInfo = rdao.selectRentbookinfo(commandMap);
		
		for(RentBook book : curRentInfo) {
			
			int extendCnt = Integer.parseInt(book.getExtentionCnt());

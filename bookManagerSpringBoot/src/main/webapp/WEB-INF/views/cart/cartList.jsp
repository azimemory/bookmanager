...
<div class="content">	
	...
	<c:forEach items="${data}" var="cart" varStatus="i">
	<div class="box_rent-detail" id="brd_${i.count}">
  	    <div class="book-detail">
  	    	<input type="checkbox" data-title="${cart.book.title}" name="bIdx" class="chk_multi-rent" value="${cart.book.bIdx}" id="cart-check" class="cart-check">
			<div class="img-book-detail">
				<img src="<%= request.getContextPath() %>/resources/image/book/${cart.book.isbn}.jpg"/>
			</div>
			<div class="book-info">
				<div>
					<div class="info-col">도서명</div>
					<div class="info-val">${cart.book.title}</div>
				</div>
				<div>
					<div class="info-col">작가</div>
					<div class="info-val">${cart.book.author}</div>
				</div>
				<div>
					<div class="info-col">isbn</div>
					<div class="info-val">${cart.book.isbn}</div>
				</div>
			</div>
			<c:choose>
			<c:when test="${cart.book.bookAmt > 0}">
			<div class="wrap_use">
				<div>
					<button class="btn_extend" 
					onclick=
					"location.href='<%= request.getContextPath() %>/rent/rent.do?bIdx=${cart.book.bIdx}&title=${cart.book.title}'">
					대출하기</button>
				</div>
				<div>
					<button type="button" class="btn_return" onclick="deleteCart('${cart.bcIdx}','brd_${i.count}')">삭제하기</button>
				</div>
			</div>
			</c:when>
		 	<c:otherwise>
